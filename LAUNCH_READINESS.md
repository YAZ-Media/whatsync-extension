# WhatSync 1.5.2 — release candidate, public checkout still closed
...
Final local result: **60 automated tests passed** (12 Node, 17 Deno, 31 browser), plus the previously completed disposable database checks. The browser suite includes a fresh-profile MV3 service-worker and popup startup check.
...
The HubSpot, billing and external-auth functions are deployed directly to WhatSync (`ogsvchujqpayuckxuwdf`). The reviewed billing, profile-privilege, checkout-reservation and Pro per-seat migrations are applied and recorded. Live checks confirm the billing schema, blocked browser role/workspace changes, restricted OAuth token reads and denied unauthenticated CRM access. Live Stripe prices are USD 19 monthly and USD 180 annually per syncing user. The live webhook and default customer portal are active. Invitations, role activation and invite acceptance enforce paid capacity; owners can change seats with Stripe proration. Public sales remain closed while the full live checkout lifecycle is tested.
...
Latest local browser run: all 31 tests passed, including the installed MV3 extension and paid-seat controls. Seventeen backend tests and 12 extension tests also passed. The real extension popup signup test verifies the shared server path and email-confirmation messaging. Signup uses a fresh server client for protected workspace creation, and database failures are reported instead of appearing successful. No live charge or authenticated CRM save/read-back has been performed.

## Gates before a green light

1. **Publish and complete acceptance.** HubSpot/billing/auth and the four reviewed migrations are deployed to the correct backend. Publish the latest `YAZ-Media/whatsync-website` main commit; the current live auth page still shows the superseded $10 message. Reload the extension from this main folder or install the candidate ZIP. Lovable Cloud function deploy buttons still target its managed backend and are not the production deployment path.
2. **Finish commercial acceptance.** The WhatSync Pro offer is approved at USD 19 per syncing user monthly or USD 180 annually. Selectable quantity, server-verified subscription state, seat enforcement, prorated seat changes, terms acceptance and the live customer portal are implemented. Validate UAE tax treatment and legal disclosures before broad paid promotion.
3. **Payment acceptance.** Sign into a non-operator WhatSync workspace and perform one authorized live checkout. Verify the signed webhook, subscription persistence, CRM entitlement, seat increase, cancellation and refund/read-back in Stripe and the owner console. No live charge or refund has been performed, so public checkout remains closed.
