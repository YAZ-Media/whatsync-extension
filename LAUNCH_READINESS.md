...
The HubSpot, billing and external-auth functions have now been deployed directly to WhatSync (`ogsvchujqpayuckxuwdf`). The reviewed billing, profile-privilege, checkout-reservation and Pro per-seat migrations are applied and recorded. Live checks confirm the billing schema, blocked browser role/workspace changes, and restricted OAuth token reads. Stripe test credentials are stored in the correct backend; the approved $19 monthly and $180 annual recurring prices and a dedicated customer portal are configured. A signed sandbox `invoice.paid` event was accepted with HTTP 200. Public sales remain closed while the full WhatSync checkout lifecycle is tested.
...
2. **Finish commercial acceptance.** The WhatSync Pro offer is approved at USD 19 per syncing user monthly or USD 180 annually. Test prices, selectable seat quantity, verified quantity persistence and the dedicated portal are implemented. The card-free 14-day trial, post-purchase seat-cap enforcement and automated seat-change proration remain launch gates. Validate UAE tax treatment and legal disclosures before live checkout.
