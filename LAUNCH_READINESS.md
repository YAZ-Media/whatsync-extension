...
2. **Confirm the paid offer and finish provider configuration.** Stripe test mode is connected to the confirmed YAZ MARKETING LLC merchant account and signed webhook delivery works. Approve plan prices, currency, monthly/yearly period, per-workspace/per-seat unit, trial, taxes, refunds and cancellation terms. Configure matching recurring price IDs and a WhatSync-specific customer portal. Three plan names are supported; differentiated seat/usage entitlements are not implemented. Do not sell quotas or premium tiers that the server does not enforce.
