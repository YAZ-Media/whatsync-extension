# WhatSync 1.5.3 — production deployment, launch blockers tracked
...
1. **Cut over the public domain.** DigitalOcean is healthy on website commit `32008c4`, but `whatsync.io` still resolves through Namecheap's old nameservers to the previous deployment. In Namecheap, replace `dns1.registrar-servers.com` and `dns2.registrar-servers.com` with `corey.ns.cloudflare.com` and `dana.ns.cloudflare.com`, with DNSSEC off during propagation.
2. **Complete store verification.** Chrome Web Store review is pending. Google Payments rejected the address document because its address did not match the publisher profile; upload a current accepted document with the same legal address. The HubSpot listing remains blocked at 2 of 3 qualifying active installs and needs one independent production customer install with real app activity.
