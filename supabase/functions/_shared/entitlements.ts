...
export async function getWorkspaceAccess(userId:string, includeSubscription=true) {
 ...
 const {data:profile,error}=await ext.from('user_profiles').select('role,status,organization_id,email').eq('user_id',userId).maybeSingle();
 ...
 const acceptanceEmails=(Deno.env.get('BILLING_ACCEPTANCE_EMAILS')||'').split(',').map(value=>value.trim().toLowerCase()).filter(Boolean);
 const acceptanceTest=Deno.env.get('BILLING_ACCEPTANCE_TEST_ENABLED')==='true'&&!!profile.email&&acceptanceEmails.includes(profile.email.toLowerCase());
 const current=subs?.find(s=>(!live||s.livemode||acceptanceTest)&&hasPaidAccess(s.status,s.current_period_end));
