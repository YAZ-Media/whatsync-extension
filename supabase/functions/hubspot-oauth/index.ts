  ...
  try {
    ...
    switch (action) {
      ...
      case 'getSyncSettings': {
        ...
        const settingsRes = await externalQuery('hubspot_integration_settings', 'GET', {
          select: 'auto_sync_contacts,auto_create_companies,enrich_before_create,attach_message_history,auto_log_conversations,contact_owner_assignment,default_pipeline_id,default_stage_id,default_lifecycle_stage,default_lead_status,mask_phone_numbers,redact_media_files,data_retention_days,field_mappings',
          filters: { user_id: `eq.${userId}` },
        });
