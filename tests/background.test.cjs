...
test('transient refresh outage preserves the session and does not look like rejection',async()=>{
 const ctx=vm.createContext({console:quiet,getStoredRefreshToken:async()=>'refresh',fetchWithTimeout:async()=>({ok:false,status:503,text:async()=>'unavailable'}),SUPABASE_URL:'https://fixture',SUPABASE_ANON_KEY:'public'});
...
test('HubSpot error-shaped or empty contact responses are never a saved contact',async()=>{
 const ctx=vm.createContext({console:quiet,callHubSpotEdgeFunction:async()=>({success:true})});vm.runInContext(functions(['createHubSpotContactViaEdgeFunction']),ctx);
